...
function high__Cardio(){
	var highC = new array();
	highC[0] = "Bike workout:Cycle at a low resistance for a three-minute warm up at 70-80rpm.At minute three, increase rpm to above 90.Every 30 seconds increase the resistance by one level and maintain an rpm above 90.Continue until you can no longer hold the rpm above 90.Allow three minutes recovery and repeat for desired number of sets (beginners: one, advanced: three).";
	highC[1] = "Rowing machine workout: Row at a comfortable pace for a three-minute warm up.Set the screen to display calories. At the end of the third minute explode into maximum effort with the aim of burning as many calories as possible in 60 seconds.Ease off for 60 seconds. Then complete another 60-second set.Continue for desired number of sets (beginners: three, advanced: 10)";
...
function low_Intensity(){
	...
	lowI[0] = "Roller Chair Workout:The only tool you need is just a chair with wheels.Listen to music (optional) and try to move your legs or arms or use both at same time while you are setting on the chair. Use one minute as a start point and then try to rise the minutes you exercise. 
	";
	lowI[1] = "Bed Dancing/Chair Dancing/Ball Dancing:Use a bed, chair or ball as an area to exercise and play music. Start dancing using different movements with your arms and feet, move your hips up and down or in a movement of figure 8. Try some wrist or ankle weights. Repeated exercise would improve your body strength. 
	";
	
	
	var ranNumI = Math.floor(Math.random()*lowI.length)+ 0);
	return lowI[ranNumI];
}
function low_Cardio(){
	var lowC = new array();
	lowC[0] = "Bed Dancing/Chair Dancing/Ball Dancing: Use a bed, chair or ball as an area to exercise and play music. Start dancing using different movements with your arms and feet, move your hips up and down or in a movement of figure 8. Try some wrist or ankle weights. Repeated exercise would improve your body strength. 
	";
	lowC[1] = "Aerobic and Resistance Cleaning: Tool you need is just a vacuum or sweeper. The more heavy the vacuum is, the more strength needed to push or pull. Apply cleaning a room per day using the vacuum. If you suffer from walking disability, the sweeper will act as a supporter. In the beginning of practicing the exercise, you may find it is difficult for one room, but repeated exercise would help you to continue.
	";
	lowC[2] = "Water workout: Practicing exercise in the water is suitable for beginners in fitness or who just recovered from injuries. You can exercise in the water from a full aqua aerobic session to walking on underwater. If you wonder how the water exercise can help in terms of burning calories, it is important to note that the more depth of the water, the more calories burn because of the demand energy to push your body.";
		
 	var ranNumC = Math.floor(Math.random()*lowC.length)+ 0);
	return lowC[ranNumC];
	
}
function low_Strengh(){
	var lowS = new array();
	lowS[0] = "Swimming It is suitable for all because it is non-load bearing exercise. With longer swimming, calories will be more reduced.
	";
	lowS[1] = "Yuga: It is good not only for relaxing or strengthens your muscle. It burns calories with just breathing and posses for certain amount of time. It is appropriate for all levels of fitness. It burns about 200 calories for only half an hour session.
	";
	lowS[2] = "Walking: it is a simplest and affordable way for all people to burn calories. If you are not used to walk for long distances, start with a short amount of time and limit your speed. The number of calories burnt depends on your size, speed and distance you walk. With walking you can avoid joint jarring problems if you have one, which might occur when you run.
	";
		
	 var ranNumS = Math.floor(Math.random()*lowS.length)+ 0);
	return lowS[ranNumS];
}
